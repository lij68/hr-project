		...
		onRowClicked : function(node) {
			...
			$.ajax({
				...
				success : function(data) {
					...
					updatedEmpBean = $.extend(true, {}, data.emptyLicenseInfoBean); // 변경된 내용이 들어갈 공간에 딥카피
...
function showEmpListNameGrid() {
	var columnDefs = [ 
		{ headerName : "사원코드", field : "empCode" , width:130 , checkboxSelection : true}, 
...
function showWorkInfoListGrid() {
	var columnDefs = [ 
		...
		{headerName : "적용일", field : "workInfoDays", editable : true, 
		...
		{headerName : "고용형태", field : "employmentType",editable : true, cellEditor: 'agSelectCellEditor', cellEditorParams: {values: ['정규직', '계약직','일용직','파겨직','재택근로자','단시간근로자']}, },
...
function showLicenseInfoListGrid() {
	var columnDefs = [ 
		...
		{headerName : "취득일",field : "getDate",cellEditor : 'datePicker'},
		{headerName : "만료일",field : "expireDate",cellEditor : 'datePicker'},
